...
public class Context implements Serializable {
	...
	/**
	 * Constructs a new Context from a Context and asserts a hypothesis <i>hyp</i>
	 * in it.
	 *
	 ...
	 */
	protected Context(Context c, int hyp)
			throws NotAPropositionNodeException, DuplicatePropositionException, NodeNotFoundInNetworkException {
		this.names = c.getNames();
	...
	/**
	 * Constructs a new Context from an old Context <i>c</i>, and adds a new name to
	 * this new Context
	 *
	 * @param contextName
	 * @param c
	...
	/**
	 * Constructs a new Context using its name and sets the Context's hyps to a
	 * passed Proposition set <i>hyps</i>.
	 *
	 ...
	 * @return <code>true</code> if the proposition exists, otherwise
	 *         <code>false</code>
	 * @throws NotAPropositionNodeException   If the node p is not a proposition.
	 * @throws NodeNotFoundInNetworkException If the node p doesn't exist in the
	 *                                        network.
	 */
	public boolean isAsserted(PropositionNode p) throws NotAPropositionNodeException, NodeNotFoundInNetworkException {
		int hyp = p.getId();

		return Arrays.binarySearch(PropositionSet.getPropsSafely(this.hyps), hyp) > 0 || isSupported(p);
	}

	public boolean isSupported(PropositionNode node) {
		Collection<PropositionSet> assumptionSet = node.getBasicSupport().getAssumptionBasedSupport().values();
	 ...
	 * @return <code>true</code> if the name isn't a duplicate <code>false</code>
	 *         otherwise.
	 */
	 ...
	 * @return <code>true</code> if one at least one of the names isn't a duplicate
